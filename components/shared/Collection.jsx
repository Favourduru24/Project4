...
const Collection = ({data, emptyTitle, emptyStateSubtext, page, collectionType, limit, totalPages, urlParamName}) => {
  return (
    <>
     {data?.length > 0 ? (
           <div className="flex flex-col items-center gap-5 grid">
             <ul className="gap-5 grid xl:gap-5 grid-cols-[repeat(auto-fill,minmax(300px,1fr))]">
                 {data?.map((event) => {
